...
:- dynamic poi/6.
:- dynamic pontoPartida/1.
:- dynamic horaInicial/1.
:- dynamic tempoVisita/1.
:- dynamic circuitoFechado/1.
:- dynamic arco/3.
:- dynamic poi/5.
...
estimativa(Origem,Destino,Estimativa):-
	%get nos from database
	gpsCoordinate(Origem,XO,YO,_),
	gpsCoordinate(Destino,XD,YD,_),
	DX is XO - XD,
	DY is YO - YD,
...
calculaTempoDeslocacao(Peso,HoraDia,TempoDeslocacao,HoraComDeslocacao):-
	...
	DeslocacaoMin is ((Peso / 30) / Velocidade ) * 60,
...
calculaPesoMaximo(Peso):-
	velocidade_media(Velocidade), tempoVisita(Tempo),
	Peso is (Tempo * Velocidade * 30) - 100.
